...
object IPApp {
  ...
  def testImageClassification(sc: SparkContext) = {
    ...
    val path = "files/101_ObjectCategories/ant/key01.jpg"
  ...
  def main(args: Array[String]) {
    ...
    testImagesArray.foreach(f => {
      ...
      println(s"Predicting test image : " + cat + " as " + IMAGE_CATEGORIES(predictedClass.toInt))
       var i= 1
      while (i >0) {
        SocketClient.sendCommandToRobot("Image Identified as : " + cat + "\n") //+ " as " + IMAGE_CATEGORIES(predictedClass.toInt))
        i= i-1
      }
