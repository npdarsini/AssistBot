...
object IPApp {
  ...
  //val IMAGE_CATEGORIES = List( "bibimap" , "jiaozi", "oden", "omelet", "rice", "sausage", "spaghetti", "sushi", "tempura", "toast" )

  //val IMAGE_CATEGORIES = List( "cars", "bike", "motorcycle", "bus", "cruise", "train")
  val IMAGE_CATEGORIES = List( "charger", "keys", "phone", "spects", "watch")
  ...
  def testImageClassification(sc: SparkContext) = {
    ...
    val path = "files/101_ObjectCategories/ant/key1.jpg"
  ...
  def classifyImage(sc: SparkContext, path: String): Unit = {
    ...
    val p = nbModel.predict(histogram)
    println(s"Predicting test image : " + IMAGE_CATEGORIES(p.toInt))
    SocketClient.sendCommandToRobot("Image Identified as : " +IMAGE_CATEGORIES(p.toInt)+ "\n")
  ...
  def main(args: Array[String]) {
    ...
    //val ip = LocalIP.getHostAddress

    val lines = ssc.socketTextStream(ip, 2100 )
